#pragma once
#include "external/gauss/line.h"
#include "external/gauss/triangle.h"
#include "external/potential.h"
...
inline __device__ float3 local_to_global(float x1, float x2, float3 *v)
{
    return (1 - x1 - x2) * v[0] + x1 * v[1] + x2 * v[2];
}

// unit triangle (0, 0), (1, 0), (1, 1)
inline __device__ float3 local_to_global2(float x1, float x2, float3 *v)
{
    return (1 - x1) * v[0] + (x1 - x2) * v[1] + x2 * v[2];
}

inline __device__ complex singular_point(float xsi,
                                         float eta1,
                                         float eta2,
                                         float eta3,
                                         float weight,
...
inline __device__ complex singular_integrand(float3 *trial_v,
                                             float3 *test_v,
...
inline __device__ complex regular_integrand(float3 *trial_v,
                                            float3 *test_v,
                                            float trial_jacobian,
                                            float test_jacobian,
                                            complex s,
...
template <class Tri>
inline __device__ complex face2FaceIntegrand(const Tri &src, const Tri &trg, complex k, PotentialType type)
{
...
template <class Tri>
inline __device__ complex face2PointIntegrand(const Tri &src, float3 trg, complex k, PotentialType type)
{
    float3 src_v[3] = {{src.v0}, {src.v1}, {src.v2}};
    float src_jacobian = jacobian(src_v);
